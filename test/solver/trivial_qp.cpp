...
public:
    ...
    void eval_f(bool, const FixedVector<f64> &x, f64 &obj) override { obj = x[0] * x[0] + x[1] * x[1]; }

    void eval_g(bool, const FixedVector<f64> &x, FixedVector<f64> &g) override { g[0] = x[0] + x[1]; }
    ...
    void eval_jac_g(bool, const FixedVector<f64> &, const FixedVector<int> &, const FixedVector<int> &, FixedVector<f64> &jac) override {
        jac[0] = 1.0;
        jac[1] = 1.0;
    }

    void eval_hes(bool, const FixedVector<f64> &, bool, const FixedVector<f64> &, f64 obj_factor, const FixedVector<int> &, const FixedVector<int> &, FixedVector<f64> &hes)
        override {
        hes[0] = 2.0 * obj_factor;
        hes[1] = 2.0 * obj_factor;
    }

    void finalize_solution(::NLP::ReturnCode return_code, f64 opt_obj, const FixedVector<f64> &opt_x, const FixedVector<f64> &, const FixedVector<f64> &, const FixedVector<f64> &)
        override {
        ret = return_code;
        objective = opt_obj;
        solution = opt_x;
...
int main(int, char **) {
    ...
    const bool solution_ok = close(qp.solution[0], 0.5, 1e-6) && close(qp.solution[1], 0.5, 1e-6) && close(qp.objective, 0.5, 1e-6);

    if (!status_ok || !solution_ok) {
        std::cerr << "Unexpected trivial QP result: ret=" << static_cast<int>(qp.ret) << ", objective=" << qp.objective << ", x=" << qp.solution[0] << ", y=" << qp.solution[1]
                  << '\n';
        return 1;
    }
