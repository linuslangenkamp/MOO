...
static optional_value_t globl_xu0_fixed[XU_SIZE] = {{1.5, true}, {0, false}};
static optional_value_t globl_xuf_fixed[XU_SIZE] = {{1, true}, {0, false}};
...
static eval_structure_t globl_lfg_eval = {.buf_index = (int[]){0, 1}};

static coo_t globl_lfg_jac = {.row = (int[]){0, 0, 1, 1, 1}, .col = (int[]){0, 1, 0, 1, 2}, .buf_index = (int[]){0, 1, 2, 3, 4}, .nnz = 5};

static coo_t globl_lfg_lt_hes = {.row = (int[]){0, 1}, .col = (int[]){0, 1}, .buf_index = (int[]){0, 1}, .nnz = 2};

static eval_structure_t globl_mr_eval = {.buf_index = (int[]){}};

static coo_t globl_mr_jac = {.row = (int[]){}, .col = (int[]){}, .buf_index = (int[]){}, .nnz = 0};

static coo_t globl_mr_lt_hes = {.row = (int[]){}, .col = (int[]){}, .buf_index = (int[]){}, .nnz = 0};

// === simulation sparsity & functions ===

static coo_t globl_ode_jac = {.row = (int[1]){0}, .col = (int[1]){0}, .buf_index = (int[1]){0}, .nnz = 1};
...
static void
hes_mr(const f64 *xu0, const f64 *xuf, const f64 *p, const f64 *lambda, const f64 obj_factor, f64 t0, f64 tf, const f64 *data_t0, const f64 *data_tf, f64 *out, void *user_data) {}

// === objects ===

static c_callbacks_t globl_callbacks = {eval_lfg,
                                        jac_lfg,
                                        hes_lfg,
                                        eval_mr,
                                        jac_mr,
...
static mesh_ref_ctx_t globl_mesh_ctx = {.initial_intervals = 50, .nodes_per_interval = 3, .l2bn_p1_it = 2, .l2bn_p2_it = 3, .l2bn_p2_lvl = 0.0};

static solver_ctx_t globl_solver_ctx = {.derivative_test = false};

static c_problem_t globl_c_problem = {.x_size = X_SIZE,
                                      .u_size = U_SIZE,
