...
struct MOO_EXPORT ProblemConstants {
    ...
    ProblemConstants(bool has_mayer,
                     bool has_lagrange,
...
struct MOO_EXPORT FullSweepLayout {
    ...
    FullSweepLayout(bool lagrange_exists, int size_f, int size_g)
    ...
    inline int size() { return (L ? 1 : 0) + f.int_size() + g.int_size(); };
...
struct MOO_EXPORT FullSweepBuffers {
    ...
    FullSweepBuffers(FullSweepLayout &layout_lfg, HessianLFG &hes, ParameterHessian &pp_hes, const ProblemConstants &pc)
...
public:
    FullSweep(FullSweepLayout &&layout_in, const ProblemConstants &pc_in)
    ...
    inline const f64 *get_xu_ij(const f64 *xu_nlp, int i, int j) { return xu_nlp + pc.xu_size * pc.mesh->acc_nodes[i][j]; }

    inline const f64 *get_lambda_ij(const f64 *lambda, int i, int j) { return lambda + pc.fg_size * pc.mesh->acc_nodes[i][j]; }

    inline f64 *get_eval_buffer(int i, int j) { return buffers.eval.raw() + buffers.eval_size * pc.mesh->acc_nodes[i][j]; }

    inline f64 *get_jac_buffer(int i, int j) { return buffers.jac.raw() + buffers.jac_size * pc.mesh->acc_nodes[i][j]; }

    inline f64 *get_hes_buffer(int i, int j) { return buffers.hes.raw() + buffers.hes_size * pc.mesh->acc_nodes[i][j]; }
    ...
    inline size_t get_hes_buffer_size() { return buffers.hes.size(); }

    inline size_t get_hes_pp_buffer_size() { return buffers.pp_hes.size(); }

    inline void fill_zero_eval_buffer() { buffers.eval.fill_zero(); }

    inline void fill_zero_jac_buffer() { buffers.jac.fill_zero(); }
...
struct MOO_EXPORT BoundarySweepLayout {
    ...
    BoundarySweepLayout(bool mayer_exists, int size_r)
...
struct MOO_EXPORT BoundarySweepBuffers {
    ...
    BoundarySweepBuffers(BoundarySweepLayout &layout, HessianMR &hes, const ProblemConstants &pc)
...
public:
    BoundarySweep(BoundarySweepLayout &&layout_in, const ProblemConstants &pc_in)
        : layout(std::move(layout_in)),
          pc(pc_in),
    ...
    * mayer_factor is exact multiplier (no transform needed) of M */
    ...
    inline f64 *get_eval_buffer() { return buffers.eval.raw(); }

    inline f64 *get_jac_buffer() { return buffers.jac.raw(); }

    inline f64 *get_hes_buffer() { return buffers.hes.raw(); }
...
public:
    Dynamics(const ProblemConstants &pc_in, ::Simulation::Jacobian jac_pattern = ::Simulation::Jacobian::dense())
        : pc(pc_in),
    ...
    virtual void eval(const f64 *x, const f64 *u, const f64 *p, f64 t, f64 *f, void *user_data) {
        Log::error("Dynamics evaluation not implemented: use different simulation option.");
        abort();
    }
    virtual void jac(const f64 *x, const f64 *u, const f64 *p, f64 t, f64 *dfdx, void *user_data) {
        Log::error("Dynamics Jacobian not implemented: use different simulation option.");
        abort();
    }
...
class Problem {
public:
    Problem(std::unique_ptr<FullSweep> &&full, std::unique_ptr<BoundarySweep> &&boundary, std::unique_ptr<ProblemConstants> &&pc, std::unique_ptr<Dynamics> &&dynamics = nullptr)
        : full(std::move(full)),
    ...
    inline f64 lfg_pp_hes(int hes_buf_base_index) { return full->buffers.pp_hes[hes_buf_base_index]; }
    ...
    inline f64 mr_eval_r(int r_index) { return boundary->buffers.eval[boundary->layout.r[r_index].buf_index]; }

    inline f64 mr_jac(int jac_buf_base_index) { return boundary->buffers.jac[jac_buf_base_index]; }

    inline f64 mr_hes(int hes_buf_base_index) { return boundary->buffers.hes[hes_buf_base_index]; }
