...
struct BoundarySweepBuffers {
    ...
    BoundarySweepBuffers(FixedVector<FunctionMR>& mr,
                         AugmentedHessianMR& aug_hes,
                         const ProblemConstants& pc)
    : eval(FixedVector<f64>(pc.r_index_end)),
      jac(FixedVector<f64>(compute_jac_nnz_vec_fn_mr(mr))),
...
public:
    ...
    inline f64 lfg_eval(int entry, int interval_i, int node_j) {
        return full->buffers.eval[entry + full->buffers.eval_size * pc->mesh.acc_nodes[interval_i][node_j]];
    }

    inline f64 lfg_eval_L(int interval_i, int node_j) {
        assert(full->pc.has_lagrange);
        return full->buffers.eval[full->lfg[0].buf_index + full->buffers.eval_size * pc->mesh.acc_nodes[interval_i][node_j]];
    }

    inline f64 lfg_eval_f(int f_index, int interval_i, int node_j) {
        return full->buffers.eval[full->lfg[pc->f_index_start + f_index].buf_index + full->buffers.eval_size * pc->mesh.acc_nodes[interval_i][node_j]];
    }

    inline f64 lfg_eval_g(int g_index, int interval_i, int node_j) {
        return full->buffers.eval[full->lfg[pc->g_index_start + g_index].buf_index + full->buffers.eval_size * pc->mesh.acc_nodes[interval_i][node_j]];
    }
    ...
    inline f64 mr_eval_M() {
        ...
        return boundary->buffers.eval[boundary->mr[0].buf_index];
    }

    inline f64 mr_eval_r(int r_index) {
        return boundary->buffers.eval[boundary->mr[pc->r_index_start + r_index].buf_index];
    }
