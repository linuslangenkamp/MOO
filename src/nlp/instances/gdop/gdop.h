...
public:
    ...
    void finalize_solution(ReturnCode ret,
                           f64 opt_obj,
...
private:
    ...
    // hessian updates
    void accumulate_hessian_lfg(const HessianLFG &hes,
                                int interval_i,
                                int node_j,
                                const BlockSparsity &ptr_map_xu_xu,
                                const BlockSparsity &ptr_map_p_xu,
                                FixedVector<f64> &curr_hes);
    void accumulate_hessian_parameter_lfg(const ParameterHessian &pp_hes,
                                          FixedVector<f64> &curr_hes); // sum of all weighted Hessian(Lfg)_pp
    void accumulate_hessian_mr(const HessianMR &hes, FixedVector<f64> &curr_hes);
    void accumulate_hessian_from_lagrangian_gradient_lf(int interval_i, int node_j, FixedVector<f64> &curr_hes, const FixedVector<f64> &curr_lambda, f64 curr_obj_factor);
    void accumulate_hessian_from_lagrangian_gradient_lf_jac(int block_count, int interval_i, int node_j, f64 factor_tf, const JacobianLFG &jac, FixedVector<f64> &curr_hes);
    ...
    void flatten_trajectory_to_layout(const Trajectory &Trajectory, FixedVector<f64> &flat_buffer, bool from_costates);
    ...
    std::pair<std::unique_ptr<Trajectory>, std::unique_ptr<Trajectory>> finalize_optimal_bound_duals(const FixedVector<f64> &opt_z_lb, const FixedVector<f64> &opt_z_ub);
};
