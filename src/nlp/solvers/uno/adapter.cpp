...
uno_int timed_callback(f64 &total_nano, int &count, Fn &&fn) {
    ...
    fn();
    total_nano += std::chrono::duration<f64, std::nano>(std::chrono::high_resolution_clock::now() - start).count();
    count++;
    return 0;
}
...
void *UnoAdapter::create_model() {
    ...
    if (number_constraints > 0) {
        ok = ok && uno_set_constraints(model, number_constraints, &UnoAdapter::eval_g, g_lb.raw(), g_ub.raw(), nnz_jac, i_row_jac.raw(), j_col_jac.raw(), &UnoAdapter::eval_jac);
    }

    if (nnz_hes > 0) {
        ok = ok && uno_set_lagrangian_hessian(model, nnz_hes, UNO_LOWER_TRIANGLE, i_row_hes.raw(), j_col_hes.raw(), &UnoAdapter::eval_hes);
    }
...
uno_int UnoAdapter::eval_f(uno_int, const double *x, double *objective_value, void *user_data) {
    ...
    return timed_callback(adapter.callback_timing.objective_nano, adapter.callback_timing.objective_count, [&] { adapter.nlp.solver_eval_f(true, x, *objective_value); });
}
...
uno_int UnoAdapter::eval_g(uno_int, uno_int, const double *x, double *constraint_values, void *user_data) {
    auto &adapter = from_user_data(user_data);
    return timed_callback(adapter.callback_timing.constraints_nano, adapter.callback_timing.constraints_count, [&] { adapter.nlp.solver_eval_g(true, x, constraint_values); });
}

uno_int UnoAdapter::eval_jac(uno_int, uno_int, const double *x, double *jacobian_values, void *user_data) {
    auto &adapter = from_user_data(user_data);
    return timed_callback(adapter.callback_timing.jacobian_nano, adapter.callback_timing.jacobian_count, [&] { adapter.nlp.solver_eval_jac(true, x, jacobian_values); });
}

uno_int UnoAdapter::eval_hes(uno_int, uno_int, uno_int, const double *x, double objective_multiplier, const double *multipliers, double *hessian_values, void *user_data) {
    auto &adapter = from_user_data(user_data);
