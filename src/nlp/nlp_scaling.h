...
public:
    ...
    void unscale_x(const f64 *x_scaled, f64 *x_unscaled, int number_vars) override { std::memcpy(x_unscaled, x_scaled, number_vars * sizeof(f64)); }

    void unscale_g(const f64 *g_scaled, f64 *g_unscaled, int number_constraints) override { std::memcpy(g_unscaled, g_scaled, number_constraints * sizeof(f64)); }

    void unscale_f(const f64 *f_scaled, f64 *f_unscaled) override { *f_unscaled = *f_scaled; }

    void scale_f(const f64 *f_unscaled, f64 *f_scaled) override { *f_scaled = *f_unscaled; }

    void scale_g(const f64 *g_unscaled, f64 *g_scaled, int number_constraints) override { std::memcpy(g_scaled, g_unscaled, number_constraints * sizeof(f64)); }

    void scale_grad_f(const f64 *grad_unscaled, f64 *grad_scaled, int number_vars) override { std::memcpy(grad_scaled, grad_unscaled, number_vars * sizeof(f64)); }

    void scale_jac(const f64 *jac_unscaled, f64 *jac_scaled, int *i_row_jac, int *j_col_jac, int jac_nnz) override { std::memcpy(jac_scaled, jac_unscaled, jac_nnz * sizeof(f64)); }

    void scale_hes(const f64 *hes_unscaled, f64 *hes_scaled, int *i_row_hes, int *j_col_hes, int hes_nnz) override { std::memcpy(hes_scaled, hes_unscaled, hes_nnz * sizeof(f64)); }
};
...
public:
    ...
    NominalScaling(FixedVector<f64> &&x_nominal, FixedVector<f64> &&g_nominal, f64 f_nominal);
