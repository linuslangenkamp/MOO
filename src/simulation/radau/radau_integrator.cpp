...
RadauIntegrator::RadauIntegrator(/* generic Integrator */
                                 ...
                                 f64 rtol,
                                 int max_it)
    : Integrator(ode_fn, dense_output_grid, x_start_values, x_size, user_data, parameters, p_size, controls, jac_fn, jac_pattern),
      scheme(scheme),
      h_init(h_init),
      atol(atol),
      rtol(rtol),
...
extern "C" void radau_function_wrapper(int *n, f64 *t, f64 *x, f64 *f, f64 *rpar, int *ipar) {
    auto *self = static_cast<Simulation::RadauIntegrator *>(radau_integrator);
    self->get_ode(*t, x, f);
}

extern "C" void radau_dense_jac_wrapper(int *n, f64 *t, f64 *x, f64 *dfx, int *ldfx, f64 *rpar, int *ipar) {
    auto *self = static_cast<Simulation::RadauIntegrator *>(radau_integrator);
...
extern "C" void radau_dense_output(int *nr, f64 *told, f64 *t, f64 *x, f64 *cont, int *lrc, int *n, f64 *rpar, int *ipar, int *irtrn) {
