...
struct Mesh {
    Mesh(int intervals, double tf, FixedVector<double> grid, FixedVector<double> delta_t, FixedVector<int> nodes, FixedMultiVector<int, 2> acc_nodes, int node_count)
    ...
    FixedMultiVector<int, 2> acc_nodes;  // number of nodes to the left of index (i, j)

    static Mesh createEquidistantMeshFixedDegree(int intervals, double tf, int p);
    FixedMultiVector<int, 2> createAccOffsetXU(int off_x, int off_xu);
    FixedMultiVector<int, 2> createAccOffsetFG(int off_fg);
};
