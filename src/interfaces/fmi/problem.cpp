...
static Dependency make_dependency(fmi3ValueReference vref, const std::unordered_map<fmi3ValueReference, VarIndex> &vref_map) {
    auto it = vref_map.find(vref);
    if (it != vref_map.end()) {
        return {vref, it->second};
    }
    return {vref, {VarCategory::Unknown, -1, -1, -1}};
}

// read dependency list from an fmiLsDaeModelStructureHandle into a Dependency vector
static std::vector<Dependency> read_dependencies(fmiLsDaeModelStructureHandle *h, const std::unordered_map<fmi3ValueReference, VarIndex> &vref_map) {
...
static fmiLsDaeModelStructureHandle *find_output_handle(fmuHandle *fmu, const std::vector<fmi3ValueReference> &output_vrefs, fmi3ValueReference target) {
    int count = static_cast<int>(output_vrefs.size());
    for (int idx = 0; idx < count; idx++) {
        fmiLsDaeModelStructureHandle *h = fmiLsDae_getOutputByIndex(fmu, idx);
        if (h->valueReference == target) {
            return h;
        }
    }
    return nullptr;
...
void FMIData::print() {
    ...
    auto print_deps = [&](const std::string &prefix, const std::vector<fmi3ValueReference> &vrefs, const std::vector<std::vector<Dependency>> &deps_list) {
        for (size_t i = 0; i < vrefs.size(); i++) {
            ...
            Log::row(dep_fmt, fmt::format("{}[{}]", prefix, i), vrefs[i], indices_str.empty() ? "none" : indices_str);
        }
    };
...
FMIData::FMIData(FMISettings &settings)
    ...
    auto require_output = [&](uint32_t vref, const char *label) {
        auto it = std::find(priv->output_vrefs.begin(), priv->output_vrefs.end(), static_cast<fmi3ValueReference>(vref));
    ...
    if (settings.lagrange_vref) {
        require_output(*settings.lagrange_vref, "Lagrange");
    }
    if (settings.mayer_vref) {
        require_output(*settings.mayer_vref, "Mayer");
    }
    for (auto &bv : settings.path_constraint_vrefs) {
        require_output(bv.vref, "Path constraint");
    }
    for (auto &bv : settings.final_constraint_vrefs) {
        require_output(bv.vref, "Final constraint");
    }
    ...
    // build variable reference map
    {
        ...
        for (size_t i = 0; i < priv->state_vrefs.size(); i++) {
            priv->vref_map[priv->state_vrefs[i]] = {VarCategory::State, (int)i, (int)i, xu_offset++};
        }

        for (size_t i = 0; i < priv->input_vrefs.size(); i++) {
            priv->vref_map[priv->input_vrefs[i]] = {VarCategory::Input, (int)i, (int)i, xu_offset++};
        }

        for (size_t i = 0; i < priv->alg_vrefs.size(); i++) {
            priv->vref_map[priv->alg_vrefs[i]] = {VarCategory::Algebraic, (int)i, u_size + (int)i, xu_offset++};
        }

        for (size_t i = 0; i < priv->parameter_vrefs.size(); i++) {
            priv->vref_map[priv->parameter_vrefs[i]] = {VarCategory::Parameter, (int)i, (int)i, (int)i};
        }
    }
    ...
    // variable vectors
    for (auto v : priv->state_vrefs) {
        priv->xuz_vrefs.push_back(v);
    }
    for (auto v : priv->input_vrefs) {
        priv->xuz_vrefs.push_back(v);
    }
    for (auto v : priv->alg_vrefs) {
        priv->xuz_vrefs.push_back(v);
    }
    for (auto v : priv->parameter_vrefs) {
        priv->p_vrefs.push_back(v);
    }
...
void FMIData::initialize(f64 t_start, f64 t_stop) {
    fmi3Status status = fmi3_enterInitializationMode(priv->instance,
                                                     fmi3False,
                                                     0.0,     // toleranceDefined, tolerance
                                                     t_start, // startTime
                                                     fmi3True,
                                                     t_stop // stopTimeDefined, stopTime
    );
...
std::vector<f64> FMIData::get_initial_states() {
    if (priv->state_vrefs.empty()) {
        return {};
    }

    std::vector<f64> states(priv->state_vrefs.size());

    fmi3Status status = fmi3_getFloat64(priv->instance, priv->state_vrefs.data(), priv->state_vrefs.size(), states.data(), states.size());
...
void FMIData::eval_point_lfg(const f64 *xu, const f64 *p, f64 time, f64 *out) {
    ...
    fmi3_setFloat64(priv->instance, priv->xuz_vrefs.data(), priv->xuz_vrefs.size(), xu, priv->xuz_vrefs.size());
    ...
    if (has_L && settings.lagrange_expr.empty()) {
        fmi3_getFloat64(priv->instance, priv->Lfg_vrefs.data(), 1, out + off_L, 1);
    } else if (!settings.lagrange_expr.empty()) {
    ...
    if (n_gc > 0) {
        fmi3_getFloat64(priv->instance, priv->Lfg_vrefs.data() + off_gc, n_gc, out + off_gc, n_gc);
    }
}
...
void FMIData::jac_point_lfg(const f64 *xu, const f64 *p, f64 time, f64 *out) {
    ...
    auto get_partial = [&](fmi3ValueReference val_vref, fmi3ValueReference indep_vref) -> f64 {
        ...
        fmi3Status s = fmi3_getDirectionalDerivative(priv->instance, &val_vref, 1, &indep_vref, 1, &delta, 1, &result, 1);
...
void FMIData::eval_point_mrf(const f64 *xuf, const f64 *p, f64 tf, f64 *out) {
    ...
    fmi3_setFloat64(priv->instance, priv->xuz_vrefs.data(), priv->xuz_vrefs.size(), xuf, priv->xuz_vrefs.size());

    // evaluation of model
    fmi3_getContinuousStateDerivatives(priv->instance, work.data(), work.size());

    fmi3_getFloat64(priv->instance, priv->Mrf_vrefs.data(), priv->Mrf_vrefs.size(), out, priv->Mrf_vrefs.size());
}
...
void FMIData::eval_point_r0(const f64 *xu0, const f64 *p, f64 t0, f64 *out) {
    ...
    fmi3_setFloat64(priv->instance, priv->xuz_vrefs.data(), priv->xuz_vrefs.size(), xu0, priv->xuz_vrefs.size());

    // evaluation of model
    fmi3_getContinuousStateDerivatives(priv->instance, work.data(), work.size());

    fmi3_getFloat64(priv->instance, priv->r0_vrefs.data(), priv->r0_vrefs.size(), out, priv->r0_vrefs.size());
}
...
GDOP::ProblemConstants create_problem_constants(FMIData &fmi_data) {
    ...
    return GDOP::ProblemConstants(priv.has_mayer,
                                  priv.has_lagrange,
                                  std::move(x_bounds),
                                  std::move(u_bounds),
                                  std::move(p_bounds),
...
FullSweep::FullSweep(GDOP::FullSweepLayout &&layout_in, const GDOP::ProblemConstants &pc, FMIData &fmi_data)
    : GDOP::FullSweep(std::move(layout_in), pc),
      fmi_data(fmi_data) {}

BoundarySweep::BoundarySweep(GDOP::BoundarySweepLayout &&layout_in, const GDOP::ProblemConstants &pc, FMIData &fmi_data)
...
void layout_mrf_init_jac(GDOP::BoundarySweepLayout &layout, FMIData &fmi_data, GDOP::ProblemConstants &pc) {
    auto &priv = *fmi_data.priv;
    int buf_idx = 0;
    int mr_row = 0;
