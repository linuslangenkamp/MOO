...
private:
    ...
    void traverse_node(const TimingNode *node, const TimingNode *parent, const TimingNode *next, visitor_fn &visitor) const;
};
...
constexpr f64 s_to_nano(f64 seconds) {
    return seconds * 1e9;
}
constexpr f64 nano_to_ms(f64 seconds) {
    return seconds * 1e-6;
}
