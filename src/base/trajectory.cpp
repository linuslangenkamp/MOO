...
void ControlTrajectory::interpolate_at_polynomial(f64 t_query, f64 *interpolation_values) const {
    ...
    for (size_t u_idx = 0; u_idx < u.size(); u_idx++) {
        ...
        interpolation_values[u_idx] = fLGR::interpolate(mesh_p_order, true, values_i, 1, t_start, t_end, t_query);
    }
...
int CostateTrajectory::to_csv(const std::string &filename, bool write_header) const {
    std::vector<DynamicField> dynamics;
    if (!costates_f.empty()) {
        dynamics.push_back({"λ_f", costates_f});
    }
    if (!costates_g.empty()) {
        dynamics.push_back({"λ_g", costates_g});
    }

    std::vector<StaticField> statics;
    if (!costates_r.empty()) {
        statics.push_back({"λ_r", costates_r});
    }
...
CostateTrajectory CostateTrajectory::from_csv(const std::string &filename) {
    ...
    return CostateTrajectory(t_new,
...
CostateTrajectory CostateTrajectory::interpolate_onto_mesh(const Mesh &mesh) const {
    switch (interpolation) {
        ...
        case InterpolationMethod::POLYNOMIAL:
            if (!inducing_mesh) {
                Log::warning(
                    "CostateTrajectory in interpolate_onto_mesh() not induced from a Mesh. Can't perform polynomial interpolation: fallback to interpolate_onto_mesh_linear().");
                return interpolate_onto_mesh_linear(mesh);
            } else {
                return interpolate_onto_mesh_polynomial(mesh);
            }
...
void CostateTrajectory::print() {
    print_trajectory(t, {{"λ_f", costates_f}, {"λ_g", costates_g}}, "λ_r", costates_r);
}

void CostateTrajectory::print_table() {
    print_trajectory_table(t, {{"λ_f", costates_f}, {"λ_g", costates_g}}, "λ_r", costates_r, "CostateTrajectory Table");
}

// === helpers for Dual and standard Trajectory ===

bool check_time_compatibility(const std::vector<f64> &t_vec, const std::vector<std::vector<std::vector<f64>>> &fields_to_check, const Mesh &mesh) {
...
std::vector<f64> interpolate_polynomial_onto_grid_single(const Mesh &mesh, const std::vector<f64> &values, const std::vector<f64> &time_grid) {
    ...
    for (size_t k = 0; k < time_grid.size(); k++) {
        ...
        while (curr_mesh_interval + 1 < mesh.intervals && mesh.grid[curr_mesh_interval + 1] < t_query) {
        ...
        new_values[k] = fLGR::interpolate(mesh_p_order, true, values_i, stride, t_start, t_end, t_query);
    }

    return new_values;
}

std::vector<std::vector<f64>> interpolate_polynomial_onto_grid_multiple(const Mesh &mesh, const std::vector<std::vector<f64>> &values, const std::vector<f64> &time_grid) {
...
std::vector<f64> interpolate_linear_single(const std::vector<f64> &old_t, const std::vector<f64> &values, const std::vector<f64> &new_t) {
...
std::vector<std::vector<f64>> interpolate_linear_multiple(const std::vector<f64> &old_t, const std::vector<std::vector<f64>> &values, const std::vector<f64> &new_t) {
...
void print_trajectory(const std::vector<f64> &t,
...
void print_trajectory_table(const std::vector<f64> &t,
                            const std::vector<std::pair<std::string, std::vector<std::vector<f64>>>> &fields,
                            const std::string &static_name,
                            const std::vector<f64> &static_field,
 ...
 */
int write_csv(const std::string &filename, const std::vector<f64> &t, const std::vector<DynamicField> &dynamics, const std::vector<StaticField> &statics, bool write_header) {
    ...
    for (const auto &dyn : dynamics) {
        size_t dim = dyn.data.empty() ? 0 : dyn.data.size();
        for (size_t var_idx = 0; var_idx < dim; var_idx++) {
            file << "," << dyn.name << "_" << var_idx;
        }
    }
    for (const auto &st : statics) {
        for (size_t var_idx = 0; var_idx < st.data.size(); var_idx++) {
            file << "," << st.name << "_" << var_idx;
        }
    }
    file << "\n";
...
void read_csv(const std::string &filename, std::vector<f64> &t, std::map<std::string, std::vector<std::vector<f64>>> &dynamics, std::map<std::string, std::vector<f64>> &statics) {
    ...
    while (std::getline(file, line)) {
        ...
        std::string cell;
        while (std::getline(strs_data, cell, ',')) {
