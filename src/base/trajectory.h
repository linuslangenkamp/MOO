...
struct MOO_EXPORT CostateTrajectory {
    ...
    CostateTrajectory(std::vector<f64> t,
                      std::vector<std::vector<f64>> costates_f,
                      std::vector<std::vector<f64>> costates_g,
                      std::vector<f64> costates_r,
                      InterpolationMethod interpolation = InterpolationMethod::LINEAR,
...
struct MOO_EXPORT PrimalDualTrajectory {
    ...
    PrimalDualTrajectory(std::unique_ptr<Trajectory> primals_, std::unique_ptr<CostateTrajectory> costates_)
        : primals(std::move(primals_)),
          costates(std::move(costates_)),
...
std::vector<f64> interpolate_linear_single(const std::vector<f64> &t, const std::vector<f64> &values, const std::vector<f64> &new_t);

std::vector<std::vector<f64>> interpolate_linear_multiple(const std::vector<f64> &t, const std::vector<std::vector<f64>> &values, const std::vector<f64> &new_t);
...
int write_csv(const std::string &filename, const std::vector<f64> &t, const std::vector<DynamicField> &dynamics, const std::vector<StaticField> &statics, bool write_header = true);
