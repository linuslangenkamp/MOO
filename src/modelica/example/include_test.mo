...
model include_test
  ...
  Real k4;
  Real k5;
  parameter Real eventTime = 3;
  parameter Real multiplicator = 5;
  input Real u(start=710, min=600.15, max = 748.15);
  Real cost1 = - x2  annotation(isMayer=true);

equation
  k1 = if time < eventTime then exp(8.86 - 20300 / 1.9872 / u)
       else multiplicator * exp(8.86 - 20300 / 1.9872 / u);
  k2 = if time < eventTime then exp(24.25 - 37400 / 1.9872 / u)
       else multiplicator * exp(24.25 - 37400 / 1.9872 / u);
  k3 = if time < eventTime then exp(23.67 - 33800 / 1.9872 / u)
       else multiplicator * exp(23.67 - 33800 / 1.9872 / u);
  k4 = if time < eventTime then exp(18.75 - 28200 / 1.9872 / u)
       else multiplicator * exp(18.75 - 28200 / 1.9872 / u);
  k5 = if time < eventTime then exp(20.70 - 31000 / 1.9872 / u)
       else multiplicator * exp(20.70 - 31000 / 1.9872 / u);
...
/*
model include_test
  Real x1(start = 1, fixed=true, nominal = 1);
  Real x2(start = 0, fixed=true, nominal = 1);
  input Real u(min=0, max=5, nominal = 2.5);
  output Real cost_m(nominal = 0.85) = -x2  annotation(isMayer=true);
  output Real cost_l(nominal = 0.85) = -x2  annotation(isLagrange=true);
equation
  der(x1) = x1 * (x1 + x2 + u^2);
  der(x2) = u;
  annotation(
...
model include_test
  ...
  output Real cost_l = 0.5 * (y + u^2)  annotation(isLagrange=true);
  Real FINAL(min=0, max=0) = 1 - y annotation(isFinalConstraint=true);
equation
  der(y) = 2 * y + 2 * u * y^0.5;
  annotation(
    experiment(StartTime = 0, StopTime = 5, Tolerance = 1e-12, Interval = 0.05),
    __OpenModelica_simulationFlags(s = "optimization", optimizerNP = "3", lv = "LOG_STDOUT,LOG_ASSERT,LOG_STATS"),
    __OpenModelica_commandLineOptions = "+g=Optimica");
end include_test;
*/
...
output Real cost1 = (w_ice - 0.515309170685596)^2
                      ...
                      + (w_tc - 0.271443000537680)^2 annotation(isMayer=true);
...
// DIFFERENTIAL EQUATION---------------------
  ...
  annotation(
    experiment(StartTime = 0, StopTime = 0.5, Tolerance = 1e-10, Interval = 0.01),
    __OpenModelica_simulationFlags(
      s = "optimization",
      optimizerNP = "3",
      noEquidistantTimeGrid="()",
