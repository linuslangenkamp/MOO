...
void include_test_eqFunction_23(DATA *data, threadData_t *threadData)
{
  ...
  if(!tmp5)
  {
    tmp0 = GreaterEq((data->localData[0]->realVars[data->simulationInfo->realVarsIndex[12]] /* $con$CONSTR OPT_CONSTR */),-1.25);
    tmp1 = LessEq((data->localData[0]->realVars[data->simulationInfo->realVarsIndex[12]] /* $con$CONSTR OPT_CONSTR */),1.25);
    if(!(tmp0 && tmp1))
    {
      tmp3 = modelica_real_to_modelica_string_format((data->localData[0]->realVars[data->simulationInfo->realVarsIndex[12]] /* $con$CONSTR OPT_CONSTR */), (modelica_string) mmc_strings_len1[103]);
